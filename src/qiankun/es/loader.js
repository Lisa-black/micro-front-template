...
function createElement (appContent, strictStyleIsolation) {
  ...
  if (strictStyleIsolation) {
    if (!supportShadowDOM) {
      ...
      const shadow = appElement.attachShadow({
        mode: 'open'
      })
...
function getRender (appName, appContent, container, legacyRender) {
  const render = function render (_a, phase) {
    const element = _a.element
    const loading = _a.loading
    ...
    const containerElement = typeof container === 'string' ? document.querySelector(container) : container // The container might have be removed after micro app unmounted.
