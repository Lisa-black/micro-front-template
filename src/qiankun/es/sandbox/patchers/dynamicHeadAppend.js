...
function getNewAppendChild () {
  ...
  return function appendChild (newChild) {
    ...
    if (element.tagName) {
      ...
      let _a = __read(args, 5)
      const appName_1 = _a[0]
      let appWrapperGetter = _a[1]
      let proxy = _a[2]
      let singular = _a[3]
      let dynamicStyleSheetElements = _a[4]
      ...
      switch (element.tagName) {
        ...
        case STYLE_TAG_NAME:
        {
          ...
          const activated = checkActivityFunctions(window.location).some(function (name) {
        ...
        case SCRIPT_TAG_NAME:
        {
          ...
          const src = _b.src
          const text = _b.text
          const fetch_1 = frameworkConfiguration.fetch
...
function getNewRemoveChild () {
  ...
  return function removeChild (child) {
    const _a = __read(args, 1)
    let appWrapperGetter = _a[0]

    const storedContainerInfo = child[attachProxySymbol]
...
function getNewInsertBefore () {
  ...
  return function insertBefore (newChild, refChild) {
    ...
    if (element.tagName) {
      // eslint-disable-next-line prefer-const
      let _a = __read(args, 4)
      const appName_2 = _a[0]
      let appWrapperGetter = _a[1]
      let singular = _a[2]
      let dynamicStyleSheetElements = _a[3]
      ...
      switch (element.tagName) {
        ...
        case STYLE_TAG_NAME:
        {
          const stylesheetElement = newChild // have storedContainerInfo means it invoked by a micro app
