...
(function () {
  function SingularProxySandbox (name) {
    ...
    const _a = this
    const sandboxRunning = _a.sandboxRunning
    const addedPropsMapInSandbox = _a.addedPropsMapInSandbox
    const modifiedPropsOriginalValueMapInSandbox = _a.modifiedPropsOriginalValueMapInSandbox
    const currentUpdatedPropsValueMap = _a.currentUpdatedPropsValueMap
    ...
    var proxy = new Proxy(fakeWindow, {
      ...
      get: function get (_, p) {
        ...
        if (typeof value === 'function' && !isConstructable(value)) {
          ...
          const boundValue_1 = value.bind(rawWindow) // some callable function has custom fields, we need to copy the enumerable props to boundValue. such as moment function.
