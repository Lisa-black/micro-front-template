...
import Todo, { MyTodo } from './Todo.vue'

const STORAGE_KEY = 'todos'
const filters: { [ key: string ]: Function } = {
  all: (todos: MyTodo[]) => todos,
  active: (todos: MyTodo[]) => todos.filter(todo => !todo.done),
  completed: (todos: MyTodo[]) => todos.filter(todo => todo.done)
}
const defalutList = [
...
export default class extends Vue {
  ...
  private toggleTodo (todo: MyTodo) {
    todo.done = !todo.done
    this.setLocalStorage()
  }

  private deleteTodo (todo: MyTodo) {
    this.todos.splice(this.todos.indexOf(todo), 1)
    this.setLocalStorage()
  }

  private editTodo ({ todo, value }: { todo: MyTodo, value: string }) {
    todo.text = value
    this.setLocalStorage()
  }
